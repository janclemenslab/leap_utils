...
from keras.layers import Input, Conv2D, Conv2DTranspose, Add, MaxPooling2D, LocallyConnected2D, Multiply, ZeroPadding2D
from keras.optimizers import Adam

from leap_utils.layers import residual_bottleneck_module, UpSampling2D
...
def leap_local_cnn(img_size, output_channels, filters=64, upsampling_layers=False, amsgrad=False, summary=False):
    """
    LEAP CNN with bias layer at the end
    Creates and compiles network model.
    ...
    # localized bias branch
    sz = 4
    xb = MaxPooling2D(pool_size=sz, strides=sz, padding="same")(x_out)
    xb = LocallyConnected2D(output_channels, kernel_size=3, strides=1, padding="valid", activation="linear", kernel_initializer="glorot_normal")(xb)
    xb = UpSampling2D(size=(sz, sz), interpolation="bilinear")(xb)
    xb = ZeroPadding2D(padding=sz)(xb)
    x_out = Multiply()([xb, x_out])
    # Compile
    net = Model(inputs=x_in, outputs=x_out, name="LeapCNNlocal")
    net.compile(optimizer=Adam(amsgrad=amsgrad), loss="mean_squared_error")
...
def leap_local_cnn_split(img_size, output_channels, filters=64, upsampling_layers=False, amsgrad=False, summary=False):
    """
    LEAP CNN with bias layer at the narrowest part
    ...
    # PRIOR map branch
    # could add one more downsampling step here to reduce dimensionality of dense layer
    xb0 = Conv2D(output_channels, kernel_size=3, padding="same", activation="relu")(x3)  # reduce nb chans to nb_output_chans
    xb1 = LocallyConnected2D(output_channels, kernel_size=3, strides=1, padding="valid", activation="linear", kernel_initializer="glorot_normal")(xb0)
    xb1 = ZeroPadding2D(padding=1)(xb1)
    xb2 = UpSampling2D(interpolation="bilinear")(xb1)
    # if upsampling_layers:
    #     xb2 = UpSampling2D(interpolation="bilinear")(xb1)
    # else:
    #     xb2= Conv2DTranspose(output_channels, kernel_size=3, strides=2, padding="same", activation="relu", kernel_initializer="glorot_normal")(xb1)
    xb2 = Conv2D(output_channels, kernel_size=3, padding="same", activation="relu")(xb2)
    xb2 = Conv2D(output_channels, kernel_size=3, padding="same", activation="relu")(xb2)

    xb = UpSampling2D(interpolation="bilinear")(xb2)
    xb = Conv2D(output_channels, kernel_size=3, padding="same", activation="linear")(xb)

    # if upsampling_layers:
    #     xb = UpSampling2D(interpolation="bilinear")(xb2)
    #     xb = Conv2D(output_channels, kernel_size=3, padding="same", activation="linear")(xb)
    # else:
    #     xb = Conv2DTranspose(output_channels, kernel_size=3, strides=2, padding="same", activation="linear", kernel_initializer="glorot_normal")(xb2)

    # combine FEATURE amd PRIOR
    x_out = Multiply()([xb, x_out])
    # Compile
    net = Model(inputs=x_in, outputs=x_out, name="LeapCNNlocal")
    net.compile(optimizer=Adam(amsgrad=amsgrad), loss="mean_squared_error")
