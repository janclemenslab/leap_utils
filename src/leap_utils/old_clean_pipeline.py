...
def main(expID: str='localhost-20180720_182837',frame_start: int=100, frame_stop: int=300, frame_step: int=1):
    ...
    frame_range = np.arange(frame_start,frame_stop,frame_step)
    ...
    if np.mean(confmaps[1,:,:,1]) > 0.5:
        logging.info(f"   probably your confmaps are getting inverted, so I will do 1-confmaps.")
        confmaps = 1 - confmaps
    ...
    newbox_angles, bad_boxes = detect_bad_boxes_by_angle(newheads,
                                                         newtails,
                                                         epsilon=5)
    ...
    if play_boxes:
        ...
        vplay(boxes[fly_id == 0,...],frame_range[fly_frame[fly_id == 0]])
